...
public class InstantGuiElement : MonoBehaviour 
{
	...
	public virtual void  Action ()
	{
		...
		if (pointed) { pointedTime += Time.unscaledDeltaTime; unpointedTime = 0; }
		else { unpointedTime += Time.unscaledDeltaTime; pointedTime = 0; }
	...
	{	
		...
		else
		{
			...
			{
				if (pointed) pointedBlend = Mathf.Min(pointedBlend + Time.unscaledDeltaTime*style.blendSpeed, 1);
				else pointedBlend = Mathf.Max(pointedBlend - Time.unscaledDeltaTime*style.blendSpeed, 0);
